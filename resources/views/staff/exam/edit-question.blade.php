                    ...
                    <div class="col-md-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                @endif
                                <form action="{{ route('staff.questions.update', $question->id) }}" method="POST">
                                    @csrf
                                    @method('POST')
                                    <div class="form-group">
                                        <label for="question">Question</label>
                                        <textarea name="question" id="question" class="form-control" rows="5">{{ old('question', $question->question) }}</textarea>
                                    </div>
                                    <div class="form-group">
                                        <label for="option1">Option 1</label>
                                        <textarea name="option1" id="option1" class="form-control">{{ old('option1', $question->option1) }}</textarea>
                                    </div>
                                    <div class="form-group">
                                        <label for="option2">Option 2</label>
                                        <textarea name="option2" id="option2" class="form-control">{{ old('option2', $question->option2) }}</textarea>
                                    </div>
                                    <div class="form-group">
                                        <label for="option3">Option 3</label>
                                        <textarea name="option3" id="option3" class="form-control">{{ old('option3', $question->option3) }}</textarea>
                                    </div>
                                    <div class="form-group">
                                        <label for="option4">Option 4</label>
                                        <textarea name="option4" id="option4" class="form-control">{{ old('option4', $question->option4) }}</textarea>
                                    </div>
                                    <div class="form-group">
                                        <label for="answer">Answer</label>
                                        <input type="text" name="answer" id="answer" class="form-control" value="{{ old('answer', $question->answer) }}"></input>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <select name="subject_id" id="subject_id" class="form-control">
                                            ...
                                            @foreach ($subjects as $subject)
                                                <option value="{{ $subject->id }}" {{ old('subject_id', $question->subject_id) == $subject->id ? 'selected' : '' }}>{{ $subject->name }}-{{$subject->grade}}</option>
                                            @endforeach
                                        </select>
                                    </div>
